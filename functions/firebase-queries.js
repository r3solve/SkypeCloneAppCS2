...
import { getFirestore, collection, getDocs, query, where,orderBy, addDoc, serverTimestamp , or, onSnapshot} from "firebase/firestore";
...
async function getAllMessagesOfCurrentUser() {
...
async function fetchAllUsers(email){
  ...
  try {
    ...
    console.error("Error fetching users: ", err); // More descriptive error logging
    return [];
  }

}

async function uploadChat(chatId, createdBy, receiver, link, chats) {
...
async function downloadAllChats(email){
  try{
      let allChats = []
      const q = query(collection(db, "chats"), or(where("reciever", "==", email), where("createdBy", "==", email)));
      const querySnapshot = await getDocs(q);
...
async function getUserChatsRealtime(email){
...
export {db, auth,fetchAllUsers, uploadChat, downloadAllChats}
